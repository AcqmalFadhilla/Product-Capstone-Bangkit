...
interface TourismPlaceDao {
    ...
    @Query("DELETE FROM tourism_place WHERE idCategory = :id")
    fun deleteTourismPlaceWithCategoryId(id: Int)
    ...
    @Transaction
    @Query("SELECT tourism_place.placeId as placeId, tourism_place.placeName as placeName, tourism_category.categoryName as placeCategory, tourism_place.placeRating as  placeRating, tourism_place_interaction.isFavorited as isFavoritedPlace , tourism_place.placePhotoUrl as placePhotoUrl,tourism_place.placeDistance as placeDistance from tourism_place LEFT JOIN tourism_place_interaction ON tourism_place.placeId = tourism_place_interaction.placeId LEFT JOIN tourism_category ORDER BY placeId ASC")
    ...
    @Query("SELECT tourism_place.placeId as placeId, tourism_place.placeName as placeName, tourism_category.categoryName as placeCategory, placeRating as placeRating, tourism_place_interaction.isFavorited as isFavoritedPlace, placePhotoUrl as placePhotoUrl,tourism_place.placeDistance as placeDistance  from tourism_place LEFT JOIN tourism_place_interaction ON tourism_place.placeId = tourism_place_interaction.placeId LEFT JOIN tourism_category ON tourism_place.idCategory = tourism_category.categoryId WHERE tourism_place.idCategory = :idCategory ORDER BY tourism_place.placeId ASC")
